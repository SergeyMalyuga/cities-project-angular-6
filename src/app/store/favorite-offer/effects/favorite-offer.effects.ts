  ...
  public loadFavoriteOffers$ = createEffect(() =>
    this.actions$.pipe(ofType(FavoriteActions.loadFavoriteOffers),
      switchMap(() => this.favoriteOfferService.getOffers()
        .pipe(map(offers => FavoriteActions.loadFavoriteOffersSuccess({offers})),
          catchError((error: HttpErrorResponse) => of(FavoriteActions.loadFavoriteOffersFailure({error})))))));

  public toggleFavoriteOffer$ = createEffect(() =>
    this.actions$.pipe(ofType(FavoriteActions.toggleFavoriteOffer),
      switchMap(({offerId, isFavorite}) =>
        this.favoriteOfferService.toggleFavorite(offerId, isFavorite)
          .pipe(map(offer => FavoriteActions.toggleFavoriteOfferSuccess({offer})),
            catchError((error: HttpErrorResponse) => of(FavoriteActions.toggleFavoriteOfferFailure({error})))))));
}
